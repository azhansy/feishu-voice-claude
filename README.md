...
**二次确认(仅危险操作 / 切换项目时触发)**:机器人先回一张交互卡片复述「项目 + 任务」,
带 `✅ 确认执行` / `✖️ 取消` 两个按钮。**续用上次项目的普通指令(如「总结一下今天的提交」)直接执行,不再确认**;
只有以下两种情况才弹卡片等你确认:

- **切换 / 新指定项目**:防止把指令跑到错误的项目上(首次使用某项目也会确认一次)。
- **命中危险关键词**:命中 `config.json` 的 `danger.keywords`(删除/覆盖/reset --hard/push -f 等)时,卡片变红并标注 `⚠️ 疑似危险操作`。

确认卡片的操作:

- **点按钮**:点「确认执行」才真正跑;点「取消」放弃。
- **或打字**:回复「确认/确定/好的/ok」执行,「取消/算了」放弃(多张卡片并存时作用于最近一张,精确操作请点卡片按钮)。
- **多项目并发**:连发多条不同项目的指令会各弹一张卡片、互不覆盖,可分别确认后并行执行;确认有效期 5 分钟。
...
- **追问可省略项目名**:本会话上次用过哪个项目,后续不带项目名的指令就默认续用它(如「总结一下今天的提交」直接跑,不再确认)
- **切项目**:直接说项目名(推荐用英文目录名,如 `tesska …`)即可切换,并弹一次确认防跑错
- 给某个项目开新会话:发「**my-website 新会话**」(带项目名)只重置该项目;不带项目名的「重置上下文 / 新会话 / 清空记忆」重置本会话上次用的项目
- 会话映射持久化在 `state/sessions.json`、上次用的项目持久化在 `state/last_project.json`,bridge 重启后仍能续接、仍记得上次项目(不会重启后又要你重新选项目)
...
**长任务有心跳**:`claude -p` 可能跑几分钟到 `claude.timeout_seconds` 上限(默认示例 1800 秒 = 30 分钟,可自行调大;超时会杀掉整个进程组避免残留)。执行期间机器人回「⏳ [项目] 还在跑…已 X」,X 为中文时分秒(如 `1分30秒` / `2分` / `1小时`);心跳间隔从 `heartbeat_seconds`(默认 60 秒)起**逐步翻倍退避**(封顶 5 分钟),避免长任务刷屏。跑完回「📦 [项目] 结果:…」;结果超 3500 字会截断并提示去台账/日志看全文。设 `heartbeat_seconds` 为 0 关闭心跳。
